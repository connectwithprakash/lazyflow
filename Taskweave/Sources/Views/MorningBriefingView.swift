...
struct MorningBriefingView: View {
            ...
            HStack(spacing: DesignSystem.Spacing.xxxl) {
                ...
                VStack(spacing: DesignSystem.Spacing.xs) {
                    Text("\(briefing.yesterdayCompletionPercentage)%")
                        .font(.system(size: 28, weight: .bold, design: .rounded))
                        .foregroundColor(briefing.wasYesterdayProductive ? .green : Color.Taskweave.textPrimary)
                    Text("Completion")
                        .font(DesignSystem.Typography.caption1)
                        .foregroundColor(Color.Taskweave.textSecondary)
                }
            ...
            HStack(spacing: DesignSystem.Spacing.xxxl) {
                ...
                VStack(spacing: DesignSystem.Spacing.xs) {
                    Text(stats.formattedCompletionRate)
                        .font(.system(size: 24, weight: .bold, design: .rounded))
                        .foregroundColor(stats.averageCompletionRate >= 70 ? .green : Color.Taskweave.textPrimary)
                    Text("Avg Rate")
                        .font(DesignSystem.Typography.caption1)
                        .foregroundColor(Color.Taskweave.textSecondary)
                }
            ...
            ForEach(tasks.prefix(5)) { task in
                HStack(spacing: DesignSystem.Spacing.sm) {
                    ...
                    if let dueTime = task.formattedDueTime {
                        Text(dueTime)
                            .font(DesignSystem.Typography.caption1)
                            .foregroundColor(task.isOverdue ? .red : Color.Taskweave.textTertiary)
    ...
    private func priorityColor(_ priority: Priority) -> Color {
        ...
        case .none: return Color.Taskweave.textTertiary
        }
    }
